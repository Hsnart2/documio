...
function senderKey(message: InboxMessage & Partial<Analysis>) {
  const match =
    message.from.match(/<([^>]+)>/) ??
    message.from.match(/[\w.+-]+@[\w.-]+/);
  const email = (match?.[1] ?? match?.[0] ?? "").toLowerCase();
  return email.includes("@")
    ? email.split("@").pop() ?? email
    : email || message.from.toLowerCase();
}

function isSafeTrashCandidate(message: InboxMessage & Partial<Analysis>) {
  if (
    message.importance !== "low" ||
    message.suggestedAction !== "review_trash"
  ) {
    return false;
  }
  if (message.category !== "pubblicita" && message.category !== "altro") {
    return false;
  }
  if (
...
function updateEmailHero(mode: UiMode) {
  ...
  const badgeText =
    mode === "advanced"
      ? "✦ IA avanzata · Automazione protetta"
      : "✓ Azioni sempre sotto il tuo controllo";
  const paragraphText =
    mode === "advanced"
...
export default function AdvancedEmailAutomation() {
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      attempts += 1;
      if (updateEmailHero(mode) || attempts >= 24) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!token) {
          throw new Error(
            "Accedi a DocuMio per avviare l'automazione email.",
          );
        }
        ...
        if (!inboxResponse.ok) {
          throw new Error(inbox.error ?? "Lettura Gmail non riuscita.");
        }
        if (!inbox.connected || !inbox.messages?.length) {
          setStage(
            inbox.connected
              ? "Nessuna nuova email da gestire."
              : "Gmail non è ancora collegata.",
          );
          return;
        }
        ...
        for (let index = 0; index < sourceMessages.length; index += 15) {
          ...
          if (!response.ok) {
            nextReport.warnings.push(
              result?.error ?? "Un gruppo di email non è stato analizzato.",
            );
            continue;
          }
          ...
          setStage(
            ...
            )} di ${sourceMessages.length} email…`,
          );
        }
        ...
        const usefulMessageIds = enriched
          .filter(
            (message) =>
              (message.category === "documenti" ||
                message.category === "pagamenti") &&
              message.importance !== "low" &&
        ...
        const grouped = new Map<
          string,
          Array<InboxMessage & Partial<Analysis>>
        >();
        ...
        const trashIds = safeCandidates
          .filter(
            (message) =>
              ...
              (grouped.get(senderKey(message))?.length ?? 0) >= 2,
          )
          .map((message) => message.id);

        if (trashIds.length) {
          setStage(
            `Sposto ${trashIds.length} email pubblicitarie nel cestino recuperabile…`,
          );
          for (let index = 0; index < trashIds.length; index += 50) {
            ...
            if (!response.ok) {
              nextReport.warnings.push(
                result?.error ?? "Pulizia Gmail non riuscita.",
              );
              continue;
            }
            ...
            if (result?.failed?.length) {
              nextReport.warnings.push(
                `${result.failed.length} email non sono state spostate.`,
              );
            }
          }
        }
        ...
        if (!cancelled) {
          ...
          window.dispatchEvent(
            new CustomEvent("documio-email-automation-complete", {
              detail: nextReport,
            }),
          );
          window.setTimeout(() => {
            const refreshButton = Array.from(
              ...
              button.textContent?.includes("Aggiorna posta"),
            );
            refreshButton?.click();
          }, 600);
        }
        ...
        if (!cancelled) {
          setFatalError(
            error instanceof Error
              ? error.message
              : "Automazione email non riuscita.",
          );
        }
      } finally {
  ...
  const summary = report
    ? [
        ...
        .filter(Boolean)
        .join(" · ") || "Nessuna azione necessaria"
    : "";
  ...
  return createPortal(
    <aside
      className={`advanced-email-automation ${
        fatalError ? "error" : report ? "complete" : ""
      }`}
    >
      ...
      <span>
        <strong>
          <Sparkles size={14} /> Automazione IA avanzata
        </strong>
        <small>
          {fatalError || summary || stage || "Controllo automatico pronto."}
        </small>
        {report?.warnings.length ? (
          <small className="advanced-email-warning">
            {report.warnings[0]}
          </small>
        ) : null}
      </span>
